 ...
 */
function snap(val, mult, doSnap = true) {
  return !doSnap ? val : Math.ceil(val /= mult) * mult;
}
...
window.addEventListener('mousemove', event => {
  ...
  const doSnap = !event.altKey;

  if (draggingElement) {
    draggingElement.style.left = snap(event.clientX - dragStart.x, window.grid, doSnap) + "px";
    draggingElement.style.top = snap(event.clientY - dragStart.y, window.grid, doSnap) + "px";
  }

  if (resizingElement) {
    const rect = resizingElement.getBoundingClientRect();
    const origX = parseInt(resizingElement.style.left);
    const nextX = snap(event.clientX, window.grid, doSnap);
    const goalX = snap(parseInt(resizingElement.style.width) + origX - nextX, window.grid, doSnap);
    const origY = parseInt(resizingElement.style.top);
    const nextY = snap(event.clientY, window.grid, doSnap);
    const goalY = snap(parseInt(resizingElement.style.height) + origY - nextY, window.grid, doSnap);
    if (shift) { // Keep square
      ...
      if (resizeX == -1) {
        ...
        width = Math.max(minWidth, Math.min(maxWidth, snap(event.clientX - rect.left, window.grid, doSnap)));
      ...
      if (resizeY == -1) {
        ...
        height = Math.max(minHeight, Math.min(maxHeight, snap(event.clientY - rect.top, window.grid, doSnap)));
      ...
      if (resizeX == -1) {
        ...
        const width = Math.max(minWidth, Math.min(maxWidth, snap(event.clientX - rect.left, window.grid, doSnap)));
        resizingElement.style.width = width + "px";
      }
      if (resizeY == -1) {
        ...
        const height = Math.max(minHeight, Math.min(maxHeight, snap(event.clientY - rect.top, window.grid, doSnap)));
        resizingElement.style.height = height + "px";
      }
    }
  }
...
window.addEventListener('keydown', event => {
  const add = event.altKey ? 1 : event.ctrlKey ? window.grid * 5 : window.grid;
